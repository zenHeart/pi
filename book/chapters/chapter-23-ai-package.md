...
核心源码入口是 `stream()`、API registry、类型定义。`stream()` 在 [stream.ts#L25](/source-code/packages/ai/src/stream.ts#L25)，API provider registry 在 [api-registry.ts#L40](/source-code/packages/ai/src/api-registry.ts#L40)，`Context` 在 [types.ts#L333](/source-code/packages/ai/src/types.ts#L333)，`AssistantMessageEvent` 在 [types.ts#L347](/source-code/packages/ai/src/types.ts#L347)，`Model` 在 [types.ts#L538](/source-code/packages/ai/src/types.ts#L538)。
...
`stream()` 的实现很短，但非常关键。它先根据 `model.api` 找到 provider，再调用 provider 的 stream。源码在 [stream.ts#L17](/source-code/packages/ai/src/stream.ts#L17) 解析 API provider，不存在就抛 `No API provider registered for api`；[stream.ts#L25](/source-code/packages/ai/src/stream.ts#L25) 暴露统一入口；[stream.ts#L43](/source-code/packages/ai/src/stream.ts#L43) 暴露 `streamSimple()`。

API registry 用 `api` 字符串，而不是 provider 名称。`registerApiProvider()` 在 [api-registry.ts#L66](/source-code/packages/ai/src/api-registry.ts#L66) 注册某种 API 实现，例如 `openai-completions`、`openai-responses`、`anthropic-messages`、`google-generative-ai`。这解释了为什么多个 provider 可以共享同一个 API 类型，也解释了 `models.json` 里 provider 配置要声明 `api`。

内置 provider 采用 lazy registration。`packages/ai/src/providers/register-builtins.ts` 里为 Anthropic、OpenAI、Google、Mistral、Bedrock 等 provider 创建 lazy stream wrapper，导出点从 [register-builtins.ts#L326](/source-code/packages/ai/src/providers/register-builtins.ts#L326) 开始，实际注册表从 [register-builtins.ts#L348](/source-code/packages/ai/src/providers/register-builtins.ts#L348) 开始。这减少启动时加载所有 provider 实现的成本，也让 optional/provider-specific 依赖更容易隔离。

类型系统把 agent 需要的消息协议固定下来。`ToolCall` 在 [types.ts#L246](/source-code/packages/ai/src/types.ts#L246)，`AssistantMessage` 在 [types.ts#L277](/source-code/packages/ai/src/types.ts#L277)，`Tool` 在 [types.ts#L327](/source-code/packages/ai/src/types.ts#L327)。工具参数验证由 `validateToolCall()` 完成，入口在 [validation.ts#L277](/source-code/packages/ai/src/utils/validation.ts#L277)。`packages/ai/README.md` 说明 agentLoop 会自动验证工具参数；如果自己用 `stream()` 或 `complete()` 实现 loop，就要手动调用 `validateToolCall`。

模型 discovery 的底层 registry 在 `packages/ai/src/models.ts`。模块加载时把 generated `MODELS` 写入 `modelRegistry`，入口在 [models.ts#L4](/source-code/packages/ai/src/models.ts#L4)，`getModel()` 在 [models.ts#L20](/source-code/packages/ai/src/models.ts#L20)，`getProviders()` 在 [models.ts#L28](/source-code/packages/ai/src/models.ts#L28)，`getModels()` 在 [models.ts#L32](/source-code/packages/ai/src/models.ts#L32)。第 24 章的 coding-agent registry 会在这个基础上叠加 auth、models.json、extension provider。
...
第三，provider 差异仍然保留。`StreamOptions` 从 [types.ts#L84](/source-code/packages/ai/src/types.ts#L84) 开始，具体 provider 还可以有 Anthropic、OpenAI、Google 等选项。文档也有 provider-specific options section。统一入口接受通用 options，但直接调用 provider-specific stream 时 TypeScript 能约束更细的选项。
...
第三，比较 `stream()` 和 `streamSimple()`：前者直接走 provider API options，后者把简化 reasoning 等选项映射到 provider-specific options。要求你能在 [stream.ts#L25](/source-code/packages/ai/src/stream.ts#L25) 和 [stream.ts#L43](/source-code/packages/ai/src/stream.ts#L43) 解释两者共同的 provider resolution。
