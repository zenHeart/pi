...
sequenceDiagram
    ...
    TUI->>Loop: 触发 AbortController.abort()
    Note over Loop,Tool: 安全点检测：正在执行 Bash / Edit
    Loop->>Tool: 传播 AbortSignal
    Tool-->>Loop: 终止物理进程，抛出 AbortError
    Note over Loop: 捕获异常，生成合成 toolResult
    Loop->>Loop: 组装合成消息 createToolResultMessage()
    Loop->>LLM: 退出当前 Turn，将 synthetic toolResult 写入历史
