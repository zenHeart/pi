...
sequenceDiagram
    ...
    LLM->>AgentLoop: 提出 edit 申请 (edits: [{ oldText, newText }])
    AgentLoop->>Lock: withFileMutationQueue(absolutePath)
    activate Lock
    ...
    deactivate Lock
    Tool-->>AgentLoop: 返回 diffResult 和 unified patch
    AgentLoop-->>LLM: 投递 toolResult，推动下一轮决策
```
