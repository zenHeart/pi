...
Pi 的测试套件使用"假 provider"（faux provider）模拟 LLM 响应，完全不需要真实 API key。[`harness.ts`](/source-code/packages/coding-agent/test/suite/harness.ts#L92) 的 `createHarness()` 函数封装了完整的测试环境创建：
...
阅读 [`harness.ts#L130`](/source-code/packages/coding-agent/test/suite/harness.ts#L130) 的 Agent 构造部分，说明 faux provider 是如何在不修改 Agent 核心代码的情况下替换真实 LLM 调用的；解释 `getApiKey` 和 `onPayload` 参数在测试环境中的作用。
