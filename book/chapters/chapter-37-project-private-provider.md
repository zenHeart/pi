...
[`model-registry.ts#L796`](/source-code/packages/coding-agent/src/core/model-registry.ts#L796) 中的 `registerProvider()` 接受 `ProviderConfigInput`：
...
阅读 [`model-registry.ts#L860`](/source-code/packages/coding-agent/src/core/model-registry.ts#L860) 的 `applyProviderConfig()`，说明当 `config.models` 不为空时和为空时，注册逻辑的差异；解释 `oauth.modifyModels` 何时被调用，它能实现什么样的动态配置。
